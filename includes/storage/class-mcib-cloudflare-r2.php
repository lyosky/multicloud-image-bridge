...
class MCIB_Cloudflare_R2 implements MCIB_Storage_Interface {
    /**
     * Cloudflare R2配置
     *
     * @var array
    ...
    public function upload_file($file_path, $remote_path) {
        ...
        try {
            ...
            // 构建请求URL (Cloudflare R2使用S3兼容API)
            $host = "{$this->config['account_id']}.r2.cloudflarestorage.com";
            $url = "https://{$host}/{$this->config['bucket']}/{$remote_path}";
            ...
            // 创建签名字符串
            $string_to_sign = "AWS4-HMAC-SHA256\n{$date}\n{$short_date}/auto/s3/aws4_request\n" . hash('sha256', $canonical_request);
            ...
            // 构建授权头
            $authorization = "AWS4-HMAC-SHA256 Credential={$this->config['access_key']}/{$short_date}/auto/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature={$signature}";
    ...
    public function get_file_url($remote_path) {
        ...
        // 否则使用默认的R2 URL
        return "https://{$this->config['account_id']}.r2.cloudflarestorage.com/{$this->config['bucket']}/{$remote_path}";
    }

    /**
     * 测试Cloudflare R2连接是否有效
     *
     * @return bool 连接是否有效
     */
    public function test_connection() {
        ...
        try {
            ...
            // 构建请求URL
            $host = "{$this->config['account_id']}.r2.cloudflarestorage.com";
            $url = "https://{$host}/{$this->config['bucket']}/";
